...
function AddTaskButton({ newTask, Task }) {
  ...
  const deleteModalData = () => {
    ...
    setProjectSaved('');
    setDifficultySaved('');
    setSelectedDateCalendar(null);  // Limpiar la fecha seleccionada
  }
    ...
    <>
      ...
      {isModalView ? (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col justify-center items-center text-center bg-white z-101 dark:text-[#bebebe] dark:bg-[#333333] overflow-auto phone: w-80 h-150">
          ...
          {textSaved.trim() === '' || difficultyValueSelected === undefined || selectedDateCalendar === null ? (
            <h2 className="text-red-500 font-bold mb-3">No dejes espacios vacios</h2>
          ) : null}

          <div className="flex gap-10">
            <button className="rounded-full bg-red-400 w-20 h-10 mb-3 hover:bg-red-500 dark:bg-red-500 dark:hover:bg-red-600 dark:text-white cursor-pointer"
              onClick={() => { openModal(), deleteModalData() }}>Cancel</button>

            <button className="rounded-full bg-green-400 w-20 h-10 mb-3 hover:bg-green-500 dark:bg-green-500 dark:hover:bg-green-600 dark:text-white cursor-pointer"
              onClick={() => {
                if (textSaved.trim() === '' || difficultyValueSelected === undefined || selectedDateCalendar === null) return;
                newTask((previousContent) => [...previousContent, {
                  id: previousContent.length + 1,
                  taskDescription: textSaved,
                  date: selectedDateCalendar.toLocaleDateString('ja-JP'),
                  project: projectValueSelected,
                  difficult: difficultyValueSelected,
                  favorite: false,
                  completed: false
                }]);
